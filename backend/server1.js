  ...
  app.post('/sendHallTicketEmails', async (req, res) => {
    ...
    console.log(examToken, hallTicketIDs);
      ...
      for (const candidate of candidates) {
                ...
                const examLink = `https://exams.ramanasoft.com/Exam?type=global&hId=${candidate.hallTicketID}&examToken=${examToken}`;
...
//                     // Update SentHallTicketMail as '1' (success)
//                     await db.query(
//                         'UPDATE GlobalResponses SET SentHallTicketMail = ? WHERE hallTicketID = ? AND examToken = ?',
//                         [1, candidate.hallTicketID, examToken]
//                     );
//                 } else {
//                     // Update SentHallTicketMail as '0:<reason>'
//                     await db.query(
//                         'UPDATE GlobalResponses SET SentHallTicketMail = ? WHERE hallTicketID = ? AND examToken = ?',
//                         [`0:${response.message}`, candidate.hallTicketID, examToken]
//                     );
//                 }
...
app.get("/getAllQuestions/:section", async (req, res) => {
    ...
    console.log(section);
    try {
        const query = "SELECT QID, section, questionName, questionType, score, negativeMarks FROM questions where section = ?";
...
app.get('/Section_UsedIn/:section', async (req, res) => {
    const { section } = req.params;
    console.log(section);
    const query = 'SELECT * FROM exams WHERE section = ?';
...
app.post("/createGlobalExam", async (req, res) => {
    const {
        ...
        unusualBehavior, submissionType, showResults, liveSupport, PassMark
    } = req.body;
    ...
    try {
        ...
        const [examResult] = await db.query(insertExamQuery, [
            ...
            submissionType, showResults === "true", liveSupport === "true", PassMark
        ]);
...
app.post("/api/register", async (req, res) => {
    try {
        ...
        const [checkResults] = await db.query(
            `SELECT ID FROM GlobalResponses WHERE candidateEmail = ? OR candidateMobile = ?`,
            [email, mobile]
        );
...
app.get("/api/registrations", async (req, res) => {
    ...
    try {
        const [registrations] = await db.query(
            `SELECT ID, examToken, candidateName, candidateEmail, candidateMobile, hallTicketID, examStatus, createdOn 
